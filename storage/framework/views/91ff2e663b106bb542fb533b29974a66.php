<?php $__env->startSection('title', 'Dashboard - CO.DE Platform'); ?>
...
function chatUploadFlow() {
    return {
        ...
        lastUpdateTime: null,
        lastUploadedBytes: 0,
        speedSamples: [], // Per calcolo media mobile
        ...
        // Calculate SIMPLE upload speed (VELOCITÀ MEDIA DALL'INIZIO)
        calculateRealUploadStats(loadedBytes) {
            const now = Date.now();
            
            console.log(`🔍 calculateRealUploadStats called: loadedBytes=${loadedBytes}, uploadStartTime=${this.uploadStartTime}`);
            
            // PRIMA CHIAMATA - Inizializzazione SEMPLICE
            if (!this.uploadStartTime) {
                this.uploadStartTime = now;
                console.log(`🏁 INIT: Upload started at ${now}`);
                return;
            }
            
            // CALCOLO SEMPLICE: Velocità media dall'inizio
            const totalTimeSeconds = (now - this.uploadStartTime) / 1000;
            
            console.log(`⏱️ Time elapsed: ${totalTimeSeconds.toFixed(1)}s, loadedBytes: ${loadedBytes}`);
            
            // Deve essere passato almeno 0.5 secondi (ridotto per risposta più veloce)
            if (totalTimeSeconds < 0.5 || loadedBytes <= 0) {
                console.log(`⏸️ SKIP: too early or no bytes`);
                return;
            }
            
            // VELOCITÀ MEDIA TOTALE (più stabile)
            const bytesPerSecond = loadedBytes / totalTimeSeconds;
            const oldSpeed = this.uploadSpeed;
            this.uploadSpeed = bytesPerSecond / (1024 * 1024); // Converti a MB/s
            
            // TEMPO STIMATO
            if (this.uploadSpeed > 0 && this.totalBytes > loadedBytes) {
                const remainingBytes = this.totalBytes - loadedBytes;
                this.estimatedTime = (remainingBytes / (1024 * 1024)) / this.uploadSpeed;
            } else {
                this.estimatedTime = 0;
            }
            
            console.log(`⚡ SPEED UPDATE: ${oldSpeed.toFixed(3)} → ${this.uploadSpeed.toFixed(3)} MB/s`);
            console.log(`⚡ CALCULATION: ${loadedBytes} bytes ÷ ${totalTimeSeconds.toFixed(1)}s = ${bytesPerSecond.toFixed(0)} B/s = ${this.uploadSpeed.toFixed(3)} MB/s`);
        },
        
        // Upload REALE con XMLHttpRequest
        realUpload() {
            return new Promise((resolve, reject) => {
                const formData = new FormData();
                
                // Aggiungi file audio REALE
                if (this.audioFile) {
                    formData.append('audio_file', this.audioFile);
                }
                
                // Aggiungi documenti REALI
                this.documents.forEach((file, index) => {
                    formData.append(`documents[${index}]`, file);
                });
                
                // Aggiungi dati form
                formData.append('notes', this.structuredNotes || '{}');
                ...
                // XMLHttpRequest per tracking REALE
                const xhr = new XMLHttpRequest();
                
                // EVENTO PROGRESSO REALE - SEMPLIFICATO
                xhr.upload.addEventListener('progress', (event) => {
                    if (event.lengthComputable) {
                        this.uploadedBytes = event.loaded;
                        this.uploadProgress = Math.round((event.loaded / event.total) * 100);
                        
                        // Calcola velocità SEMPRE (semplice)
                        this.calculateRealUploadStats(event.loaded);
                        
                        // Log ogni 10% per debug
                        if (this.uploadProgress % 10 === 0) {
                            console.log(`📈 ${this.uploadProgress}% - Speed: ${this.uploadSpeed.toFixed(3)} MB/s - ETA: ${this.estimatedTime.toFixed(1)}s`);
                        }
                    }
                });
                ...
                xhr.addEventListener('timeout', () => {
                    reject(new Error('❌ Timeout upload (oltre 5 minuti)'));
                });
                
                // CONFIGURAZIONE REQUEST
                xhr.timeout = 300000; // 5 minuti timeout
                xhr.open('POST', this.$refs.uploadForm.action || '/upload');
                xhr.send(formData);
            });
        },
        
        // Costruisci note strutturate
        ...
        // FORM SUBMISSION con UPLOAD REALE
        async submitForm() {
            ...
            // Costruisci note prima dell'upload
            this.buildStructuredNotes();
            
            // Calcola dimensioni REALI totali
            this.totalBytes = 0;
            if (this.audioFile) this.totalBytes += this.audioFile.size;
            this.documents.forEach(file => this.totalBytes += file.size);
            
            console.log(`🚀 Inizio upload REALE - Dimensione totale: ${this.formatBytes(this.totalBytes)}`);
            
            // Mostra barra progresso e RESET SEMPLICE
            ...
            // RESET solo quello che serve
            this.uploadStartTime = null; // Sarà inizializzato alla prima chiamata
            
            console.log(`🚀 SIMPLE RESET - Upload tracking ready`);
            
            // Auto-scroll alla barra
            setTimeout(() => {
            ...
            try {
                // AVVIA UPLOAD REALE
                const response = await this.realUpload();
                
                console.log('🎉 Upload completato con successo');
                
                // Mostra completamento per un momento
                setTimeout(() => {
                    this.showUploadModal = false;
                    
                    // Gestisci risposta server
                    if (response) {
                        try {
                            ...
                            // Response non è JSON, continua
                        }
                    }
                    
                    // Comportamento default successo
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4 text-white">
                <div class="text-right">
                    <p class="text-sm opacity-90">Piano <?php echo e(ucfirst(auth()->user()->plan)); ?></p>
                    <p class="font-semibold"><?php echo e(auth()->user()->verbali_remaining); ?> verbali rimanenti</p>
                </div>
    ...
    <div class="container mx-auto px-4 max-w-4xl">
        ...
        <div class="space-y-6" x-data="{
            ...
            <div class="pl-14" x-show="currentStep >= 1 && !audioUploaded">
                <form method="POST" action="<?php echo e(route('upload')); ?>" enctype="multipart/form-data" x-ref="uploadForm">
                    <?php echo csrf_field(); ?>
...
$message = $__bag->first($__errorArgs[0]); ?>
                        <p class="text-red-500 text-sm mt-2 pl-4"><?php echo e($message); ?></p>
                    <?php unset($message);
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /app/resources/views/dashboard/upload.blade.php ENDPATH**/ ?>